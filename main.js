...
let check_for_ar = db.prepare('SELECT * FROM autoresponders WHERE prompt=? AND server=?');
let check_for_reactionrole = db.prepare('SELECT * FROM reactionroles WHERE emoji=? AND message=? AND server=?');
let check_for_reactionrole_msg = db.prepare('SELECT * FROM reactionroles WHERE message=? AND server=?');
    ...
    commands: [
        {
            ...
            responder: async (msg, cmd) => {
                ...
                try {
                    ...
                    await msg.channel.send(util_functions.desc_embed('Failed to pin: ' + e));
                }
            },
        },
        {
            ...
            responder: async (msg, cmd, client) => {
                try {
                    ...
                    let res = eval(`(async () => {${cmd.code
                        .replace('```js', '')
                        .replace('```javascript', '')
                        .replace('```', '')}})().catch(e=>msg.channel.send(\`Error: \${e}\`)).then(r=>r ? msg.channel.send(r) : 'Ran')`);
                }
                catch (e) {
        ...
        },
        {
            ...
            permissions: (msg) => msg.member && msg.member.hasPermission('MANAGE_MESSAGES'),
            responder: async (msg, cmd) => {
                if (!msg.guild)
                    throw new util_functions.BotError('user', 'No guild found');
                if (!cmd.keep)
                    util_functions.assertHasPerms(msg.guild, ['MANAGE_MESSAGES']);
                if (anonchannels.check_anon_channel.get(cmd.channel ? cmd.channel : msg.channel.id, msg.guild.id) &&
                    anonchannels.check_anon_ban.get({
                        user: msg.author.id,
                        server: msg.guild.id,
                    })) {
                    let bm = await msg.channel.send(util_functions.desc_embed(`${msg.author}, you're banned from sending messages there!`));
                    setTimeout(async () => await bm.delete(), 2000);
                }
                else {
                    if (!cmd.keep)
                        await msg.delete();
                    let chan = msg.guild.channels.cache.find((n) => n.id == (cmd.channel ? cmd.channel : msg.channel.id));
                    if (!chan || chan.type !== 'text')
                        throw new util_functions.BotError('user', 'Channel is not found!');
                    await chan.send(cmd.text);
                }
            },
        },
        {
            ...
            explanation: 'Add/Remove an anonymous channel. If no channel is provided it will use the current channel',
            matcher: (cmd) => cmd.command == 'setanonchannel',
            permissions: (msg) => msg.member && msg.member.hasPermission('MANAGE_CHANNELS'),
            responder: async (msg, cmd) => {
                ...
                if (!msg.guild)
                    if (cmd.enabled) {
                        ...
                        db.prepare('DELETE FROM anonchannels WHERE id=? AND server=?').run(channel, msg.guild.id);
                    }
                msg.channel.send(util_functions.desc_embed(`${cmd.enabled ? 'Enabled' : 'Disabled'} <#${channel}>`));
            },
        },
        {
            ...
            responder: async (msg) => {
                ...
                if (channels.length == 0) {
                    ...
                    await msg.channel.send(channels
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (author) {
                    await msg.channel.send(util_functions.desc_embed(`<@${author.user}>`));
                }
                else {
        ...
        },
        {
            ...
            responder: async (msg) => {
                ...
                let type = await util_functions.embed_options('What should I do to the original channel?', ['Delete', 'Archive', 'Nothing'], ['🗑️', '📂', '💾'], msg);
                let clone = async (type) => {
                    if (!msg.guild || !msg.guild.id)
                        return;
                    if (msg.channel.type !== 'text')
                        throw new util_functions.BotError('user', 'Not a text channel!');
                    await msg.channel.send(util_functions.desc_embed('Running clonepurge'));
                    ...
                    try {
                        for (let pin of pins) {
                            ...
                            let msg_username = pin.member
                                ...
                                : pin.author.username;
                            await (await anonhook.send(pin.content, {
                                embeds: pin.embeds,
                        ...
                        if (type === 2) {
                            ...
                            let deleted_catergory = msg.guild.channels.cache.find((n) => n.type == 'category' && n.name == 'archived');
                            ...
                            await msg.channel.send(util_functions.desc_embed('Finished. Deleting channel in 10 seconds'));
                            await sleep(10000);
                            await msg.channel.delete();
                        }
                    }
                    catch (e) {
                        await msg.channel.send(util_functions.desc_embed(`Clonepurge failed: ${e}`));
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (msg.channel.id == '707361413894504489')
                    return;
                if (await util_functions.confirm(msg)) {
                    msg.channel.send(util_functions.desc_embed('Deleting channel in 5 seconds'));
                    await sleep(5000);
                    await msg.channel.delete();
                }
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                if (!client.user || !msg.member || !msg.guild)
                    throw new util_functions.BotError('user', 'Something is seriously broken');
                util_functions.assertHasPerms(msg.guild, ['MANAGE_CHANNELS']);
                let channel = client.channels.cache.get(cmd.channel ? cmd.channel : msg.channel.id);
                if (!channel)
                ...
                if (!realchannel.permissionsFor(msg.member))
                    throw new util_functions.BotError('user', 'Something is seriously broken');
                if (!realchannel.permissionsFor(msg.member).has('VIEW_CHANNEL')) {
                ...
                await msg.channel.send(util_functions.desc_embed(`${cmd.allowed ? 'Allowed' : 'Disallowed'} ${user} ${cmd.allowed ? 'to' : 'from'} ${cmd.allowed ? 'read' : 'reading'} messages in ${channel}`));
            },
        },
        {
            name: 'archivechannel',
            syntax: 'm: archivechannel <ROLE>',
            explanation: 'Archive a channel. Users with specified role will still be able to see it',
            matcher: (cmd) => cmd.command == 'archivechannel',
            permissions: (msg) => msg.member && msg.member.hasPermission('MANAGE_CHANNELS'),
            responder: async (msg, cmd) => {
                util_functions.assertHasPerms(msg.guild, ['MANAGE_CHANNELS']);
                let deleted_category = (msg.guild.channels.cache.find((n) => n.type == 'category' && n.name == 'archived') ||
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (cmd.time) {
                    ...
                    await msg.channel.send(util_functions.desc_embed(`Banned <@${cmd.user}> for ${cmd.time}`));
                }
                else {
                    await msg.channel.send(util_functions.desc_embed(`Banned <@${cmd.user}>`));
                }
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                await msg.channel.send(util_functions.desc_embed(`Unbanned <@${cmd.user}>`));
            },
        },
        {
            name: 'tmpchannel',
            ...
            responder: async (msg, cmd) => {
                ...
                try {
                    ...
                    await msg.channel.send(util_functions.desc_embed('Failed to create channel: ' + e));
                    return;
                }
                util_functions.schedule_event({ type: 'deletechannel', channel: channel.id }, cmd.duration);
                let deletion_time = moment().add(parse_duration(cmd.duration));
                let tm_text = `Deleting channel ${deletion_time.fromNow()}`;
                let time_message = await channel.send(util_functions.desc_embed(tm_text));
                ...
                await msg.channel.send(util_functions.desc_embed(`Creating ${channel} for ${cmd.duration}`));
            },
        },
        {
            name: 'setpinperms',
            ...
            permissions: (msg) => msg.member && msg.member.hasPermission('MANAGE_ROLES'),
            responder: async (msg, cmd) => {
                util_functions.assertHasPerms(msg.guild, ['MANAGE_MESSAGES']);
                if (cmd.allowed) {
                    db.prepare('INSERT INTO pinners VALUES (?, ?)').run(cmd.role, msg.guild.id);
                    msg.channel.send(util_functions.desc_embed(`<@&${cmd.role}> are now allowed to pin messages with :pushpin:`));
                }
                else {
                    db.prepare('DELETE FROM pinners WHERE roleid=? AND guild=?').run(cmd.role, msg.guild.id);
                    msg.channel.send(util_functions.desc_embed(`<@&${cmd.role}> are no longer allowed to pin messages with :pushpin:`));
                }
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                let roles = db
                    ...
                    .all(msg.guild.id);
                msg.channel.send(util_functions.desc_embed(roles
                    .map((n) => `${n.roleid} (<@&${n.roleid}>)`)
                    .join('\n') || 'None'));
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                if (cmd.action === 'add') {
                    try {
                        await msg.channel.send('What message should this AutoResponder reply to?');
                        let prompt = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                            max: 1,
                            time: 10000,
                        });
                        ...
                        let message_type = await util_functions.embed_options('Message type?', ['Text', 'Embed'], ['📝', '🔗'], msg);
                        if (message_type === 0) {
                            await msg.channel.send('What should I reply with?');
                            let response = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                                max: 1,
                                time: 10000,
                            });
                            ...
                            if (response.array()[0].attachments.array().length) {
                                await msg.channel.send(util_functions.desc_embed('Attachments are not supported'));
                                return;
                            }
                            db.prepare('REPLACE INTO autoresponders(prompt, type, text_response, server) VALUES (?, ?, ?, ?)').run(prompt.array()[0].content, 'text', response.array()[0].content, msg.guild.id);
                        }
                        else if (message_type === 1) {
                            await msg.channel.send('What should the embed title be?');
                            let embed_title = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                                max: 1,
                                time: 10000,
                            });
                            ...
                            let embed_desc = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                                max: 1,
                                time: 10000,
                            });
                            ...
                            db.prepare('REPLACE INTO autoresponders(prompt, type, embed_title, embed_description, server) VALUES (?, ?, ?, ?, ?)').run(prompt.array()[0].content, 'embed', embed_title.array()[0].content, embed_desc.array()[0].content, msg.guild.id);
                        }
                        else {
                            return;
                        }
                        await msg.channel.send(util_functions.desc_embed('Added AutoResponder'));
                    }
                    catch (e) {
                        console.log(e);
                        await msg.channel.send(util_functions.desc_embed('Failed to create AutoResponder'));
                    }
                }
                else if (cmd.action === 'remove') {
                    await msg.channel.send('What AutoResponder would you like to remove?');
                    let prompt = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 10000,
                    });
                    ...
                    if (rc.changes)
                        await msg.channel.send(util_functions.desc_embed('Removed AutoResponder'));
                    else
                        await msg.channel.send(util_functions.desc_embed("Couldn't find AutoResponder"));
                }
                else if (cmd.action === 'list') {
                    let ars = db
                        .prepare('SELECT * FROM autoresponders WHERE server=?')
                        .all(msg.guild.id);
                    await msg.channel.send(util_functions.desc_embed(ars ? ars.map((n) => `${n.prompt}`).join('\n') : 'None'));
                }
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                try {
                    let res = await (await nodefetch('http://api.wolframalpha.com/v2/query?appid=KGQK9K-5TT39X9VQ8&input=' +
                        encodeURIComponent(cmd.text) +
                        '&format=plaintext&output=json')).json();
                    console.log(res.queryresult.pods);
                    console.log(res.queryresult.pods[1].subpods[0].plaintext);
                    msg.channel.send(util_functions.desc_embed(res.queryresult.pods[1].subpods[0].plaintext));
                }
                catch (e) {
            ...
            responder: async (msg, cmd) => {
                ...
                if (cmd.action === 'enable') {
                    if (db
                        ...
                        .get(msg.guild.id)) {
                        await msg.channel.send('This server already has a join role. You can disable it with `m: joinroles disable`');
                        return;
                    }
                    await msg.channel.send('What role would you like to set as the join role?');
                    let role = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 20000,
                    });
                    ...
                    db.prepare('INSERT INTO join_roles VALUES (?, ?)').run(msg.guild.id, role);
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (cmd.action === 'add') {
                    await msg.channel.send('What channel would you like the message to be in?');
                    let chan = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 20000,
                    });
                    ...
                    await msg.channel.send('What should the embed description be?');
                    let embed_description = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 70000,
                    });
                    ...
                    await msg.channel.send('What should the reactions be?\nFormat:\n```:grinning: @happy\n:sad: @unhappy```');
                    let reacts = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 70000,
                    });
                    ...
                    try {
                        ...
                        if (!tmp_chan || tmp_chan.type !== 'text') {
                            await msg.channel.send(util_functions.desc_embed("Channel doesn't exist!"));
                            return;
                        }
                        rr_mes = await tmp_chan.send(new discord_js_1.default.MessageEmbed()
                            .setTitle(embed_title.array()[0].content)
                            .setDescription(embed_description.array()[0].content));
                    }
                    catch (e) {
                        await msg.channel.send(util_functions.desc_embed("Couldn't send message!"));
                        return;
                    }
                    ...
                    for (let react of reacts_formatted) {
                        ...
                        if (!serv_role)
                            throw new util_functions.BotError('user', `${react.role} does not exist`);
                        if (serv_role.position >= hp) {
                            await msg.channel.send(util_functions.desc_embed('Your highest role position is below one of the roles you tried to add'));
                            return;
                        }
                    }
                    for (let react of reacts_formatted) {
                        ...
                        if (react.emoji.includes('<')) {
                            let em = msg.guild.emojis.cache.find((n) => `<:${n.name}:${n.id}>` == react.emoji);
                            if (!em) {
                                await msg.channel.send(util_functions.desc_embed('Emoji not found. You can only use emojis from this server'));
                                await rr_mes.delete();
                                return;
                            }
                        }
                    ...
                    for (let react of reacts_formatted) {
                        ...
                        if (!react.emoji.includes('<')) {
                            ...
                            db.prepare('INSERT INTO reactionroles VALUES (?, ?, ?, ?)').run(rr_mes.id, msg.guild.id, react.emoji, react.role);
                        }
                        else {
                            let em = msg.guild.emojis.cache.find((n) => `<:${n.name}:${n.id}>` == react.emoji);
                            db.prepare('INSERT INTO reactionroles VALUES (?, ?, ?, ?)').run(rr_mes.id, msg.guild.id, em.id, react.role);
                    ...
                    let mid = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                        max: 1,
                        time: 20000,
                    });
                    if (!mid.array().length) {
                    ...
                    let rr_mes;
                    try {
                        rr_mes = await msg.guild.channels.cache.get(cchan).messages.fetch(mid.array()[0].content);
                    }
                    catch (e) {
                        await msg.channel.send(util_functions.desc_embed("Couldn't find message"));
                        return;
                    }
                    await msg.channel.send('What should the embed title be?');
                    let embed_title = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                    ...
                    await rr_mes.edit(new discord_js_1.default.MessageEmbed()
                        .setTitle(embed_title.array()[0].content)
                        .setDescription(embed_description.array()[0].content));
                    await msg.channel.send('What should the reactions be?\nFormat:\n```:grinning: @happy\n:sad: @unhappy```');
                    let reacts = await msg.channel.awaitMessages((m) => m.author.id == msg.author.id, {
                    ...
                    db.prepare('DELETE FROM reactionroles WHERE message=?').run(rr_mes.id);
                    for (let react of reacts_formatted) {
                        let serv_role = msg.guild.roles.cache.get(react.role);
                        if (!react.emoji.includes('<')) {
                            await rr_mes.react(react.emoji);
                    ...
                    for (let reaction of rr_mes.reactions.cache.array()) {
                        let rr = check_for_reactionrole.get(reaction.emoji.name, reaction.message.id, reaction.message.guild.id);
                        if (!rr)
                            rr = check_for_reactionrole.get(reaction.emoji.id, reaction.message.id, reaction.message.guild.id);
                        if (!rr) {
                            reaction.remove();
                        }
                    }
        ...
        },
        {
            ...
            permissions: (msg) => msg.member && msg.member.hasPermission('KICK_MEMBERS'),
            responder: async (msg, cmd) => {
                ...
                if (kickee_hp >= hp) {
                    await msg.channel.send(util_functions.desc_embed('Your highest role is below or equal to the user you are tying to kick'));
                }
                else {
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (kickee_hp >= hp && kickee.id != msg.author.id) {
                    await msg.channel.send(util_functions.desc_embed('Your highest role is below or equal to the user you are trying to change roles on'));
                }
                else {
                    if (cmd.action === 'remove') {
                        if (!kickee.roles.cache.get(cmd.role)) {
                            await msg.channel.send(util_functions.desc_embed(`${kickee} doesn't have <@&${cmd.role}>`));
                            return;
                        }
                        ...
                        await msg.channel.send(util_functions.desc_embed(`Removed <@&${cmd.role}> from ${kickee} for ${cmd.duration}`));
                    }
                    else if (cmd.action === 'add') {
                        let role_to_be_added = msg.guild.roles.cache.get(cmd.role);
                        if (!role_to_be_added) {
                            await msg.channel.send(util_functions.desc_embed(`<@&${cmd.role}> doesn't exist`));
                            return;
                        }
                        if (hp <= role_to_be_added.position) {
                            await msg.channel.send(util_functions.desc_embed(`<@&${cmd.role}> is equal to or above you in the role list`));
                            return;
                        }
                        ...
                        await msg.channel.send(util_functions.desc_embed(`Added <@&${cmd.role}> to ${kickee} for ${cmd.duration}`));
                    }
                }
            },
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (count > 50) {
                    await msg.channel.send(util_functions.desc_embed('Must be less than or equal to 50 messages'));
                    return;
                }
                try {
                    let purged_msg_num = await msg.channel.bulkDelete(count + 1);
                    let purged_info_msg = await msg.channel.send(`Purged ${purged_msg_num.array().length - 1} messages`);
                    setTimeout(() => {
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                if (!mentioned_member)
                    throw new util_functions.BotError('user', 'User not found! Have they left the server?');
                ...
                desc.push(`${use_pronouns ? 'They' : mentioned_member} joined this server ${time_in_server}.`);
                let usernotes = db
        ...
        },
        {
            ...
            responder: async (msg, cmd) => {
                ...
                db.prepare('INSERT INTO notes VALUES (?, ?, ?, ?, ?)').run('note', cmd.text, cmd.user, msg.guild.id, id);
                await msg.channel.send(util_functions.desc_embed(`Added note to <@${cmd.user}>, note ID \`${id}\`!`));
            },
        },
        {
            name: 'warn',
            ...
            responder: async (msg, cmd) => {
                let id = nanoid.nanoid(5);
                db.prepare('INSERT INTO notes VALUES (?, ?, ?, ?, ?)').run('warn', cmd.text, cmd.user, msg.guild.id, id);
                let mentioned_member = msg.guild.members.cache.get(cmd.user);
                if (!mentioned_member)
                    throw new util_functions.BotError('user', "Can't find user! Have they left the server?");
                try {
                    await (await mentioned_member.createDM()).send(new discord_js_1.default.MessageEmbed()
                        .setTitle(`You have been warned in ${msg.guild.name}`)
                        .setDescription(`**Warn Message:**\n\`${cmd.text}\``)
                        .setFooter(`Warning ID: ${id}`));
                }
                catch (e) {
                    await msg.channel.send(util_functions.desc_embed('Alert: User will not receive a DM for this warn because they have them disabled'));
                }
                await msg.channel.send(util_functions.desc_embed(`Warned <@${cmd.user}>, warning ID ${id}`));
            },
        },
        {
            name: 'forgive',
            ...
            responder: async (msg, cmd) => {
                ...
                if (!warn_item) {
                    await msg.channel.send(util_functions.desc_embed(`Couldn't find ${cmd.id}`));
                    return;
                }
                if (warn_item.user == msg.author.id) {
                    await msg.channel.send(util_functions.desc_embed(`You can't forgive ${warn_item.type}s against yourself`));
                    return;
                }
                db.prepare('DELETE FROM notes WHERE server=? AND id=? ').run(msg.guild.id, cmd.id);
...
client.on('ready', async () => {
    ...
    if (!db.prepare('SELECT * FROM updates WHERE version=?').get(pj.version)) {
        let changes = pj.changelogs.filter((change) => !db.prepare('SELECT * FROM updates WHERE version=?').get(change.version));
        console.log(changes);
        for (let alertchannel of db.prepare('SELECT * FROM alert_channels').all()) {
            ...
            if (!ralertchannel || ralertchannel.type !== 'text')
                continue;
            await ralertchannel.send(new discord_js_1.default.MessageEmbed()
    ...
    setInterval(async () => {
        ...
        for (let event_item of events) {
            ...
            if (event.type == 'reminder') {
                try {
                    await client.channels.cache.get(event.channel).send(`<@${event.user}>, you have a reminder: ${event.text}`);
                }
                catch (e) {
                    //
                }
            ...
            if (event.type == 'anonunban') {
                try {
                    ...
                    await client.channels.cache.get(event.channel).send(util_functions.desc_embed(`Unbanned <@${event.user}>!`));
                }
                catch (e) {
                    //
                }
            ...
            if (event.type == 'removeSlowmodePerm') {
                try {
                    ...
                    db.prepare('DELETE FROM slowmoded_users WHERE user=? AND channel=?').run(event.user, event.channel);
                }
                catch (e) {
                    console.log(e);
                }
            ...
            if (event.type == 'tmprole') {
                try {
                    ...
                    if (event.action === 'add') {
                        ...
                        await channel.send(util_functions.desc_embed(`Gave <@&${event.role}> to ${user}`));
                    }
                    else {
                        await user.roles.remove(event.role);
                        await channel.send(util_functions.desc_embed(`Removed <@&${event.role}> from ${user}`));
                    }
                }
                catch (e) {
                    console.log(e);
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    try {
        ...
        if (member &&
            member.roles.cache.find((n) => roles_that_can_pin.filter((rcp) => rcp.roleid == n.id && rcp.guild == reaction.message.guild.id).length > 0) &&
            reaction.emoji.name == '📌' &&
            !reaction.message.pinned) {
            try {
                reaction.message
                    .pin()
                    .catch((e) => setTimeout(() => reaction.message.channel.send(util_functions.desc_embed(`Failed to pin: ${e}`)), 2000));
                let pm = await reaction.message.channel.awaitMessages((n) => true, //n.content.includes('pinned a message to this channel'),
                { max: 1, time: 1000 });
                if (pm.first()) {
                    await pm.first().delete();
                    await reaction.message.channel.send(util_functions.desc_embed(`${user} pinned [a message](${reaction.message.url}) to this channel`));
                }
            }
            catch (e) {
                await reaction.message.channel.send(util_functions.desc_embed(`Failed to pin: ${e}`));
            }
        }
        let rr = check_for_reactionrole.get(reaction.emoji.name, reaction.message.id, reaction.message.guild.id);
        if (!rr)
        ...
        if (!user.bot && rr) {
            ...
            try {
                ...
                if (alertchannels.check_for_alert_channel.get(reaction.message.guild.id)) {
                    let tmp = reaction.message.guild.channels.cache.get(alertchannels.check_for_alert_channel.get(reaction.message.guild.id)
                        .channel);
                    if (tmp && tmp.type == 'text')
                        tmp.send(util_functions.desc_embed(`Warning: Failed to give <@&${rr.role}> to ${user} on reaction role`));
                }
            }
        }
        else if (!user.bot &&
            check_for_reactionrole_msg.get(reaction.message.id, reaction.message.guild.id)) {
...
client.on('messageReactionRemove', async (reaction, user) => {
    ...
    try {
        ...
        if (member &&
            ...
            await reaction.message.channel.send(util_functions.desc_embed(`${user} unpinned [a message](${reaction.message.url}) from this channel`));
        }
        let rr = check_for_reactionrole.get(reaction.emoji.name, reaction.message.id, reaction.message.guild.id) ||
            check_for_reactionrole.get(reaction.emoji.id, reaction.message.id, reaction.message.guild.id);
        if (!user.bot && rr) {
            let member = reaction.message.guild.member(user);
            try {
                ...
                if (alertchannels.check_for_alert_channel.get(reaction.message.guild.id)) {
                    let tmp = reaction.message.guild.channels.cache.get(alertchannels.check_for_alert_channel.get(reaction.message.guild.id)
                        .channel);
                    if (tmp)
                        tmp.send(util_functions.desc_embed(`Warning: Failed to remove <@&${rr.role}> from ${user} on reaction role`));
                }
            }
        }
    }
...
client.on('guildMemberAdd', async (member) => {
    if (db.prepare('SELECT * FROM join_roles WHERE server=?').get(member.guild.id))
        member.roles.add(db.prepare('SELECT * FROM join_roles WHERE server=?').get(member.guild.id)
            .role);
});
...
client.on('message', async (msg) => {
    try {
        ...
        if (ap)
            await (await msg.channel.send(ap.message)).delete();
        ...
        if (msg.guild &&
            ...
            if (!anonchannels.check_anon_ban.get({
                user: msg.author.id,
                server: msg.guild.id,
            }))
                ...
                await msg.delete();
                let bm = await msg.channel.send(util_functions.desc_embed(`${msg.author}, you're banned!`));
                setTimeout(async () => await bm.delete(), 2000);
            }
        }
        ...
        if (ar) {
            if (ar.type == 'text')
                msg.channel.send(ar.text_response);
        ...
        if (msg.member &&
            ...
            await msg.channel.send(util_functions.desc_embed("Warning: You don't have an alert channel setup. This is very important for the bot to be able to warn you if there is an issue. Please set one up with `m: alertchannel enable`, or type `m: alertchannel ignore` to stop getting this message"));
        }
        if (msg.content == 'm: help') {
            ...
            msg.channel.send(new discord_js_1.default.MessageEmbed()
                ...
                .setDescription('<> means required, [] means optional. Type `m: help <NAME>` to get help for a specific command module')
                .addFields(chunks));
            return;
        }
        else if (msg.content.startsWith('m: help ')) {
            let chosen_module = all_command_modules.find((mod) => mod.title.toLowerCase() ==
            ...
            msg.channel.send(new discord_js_1.default.MessageEmbed()
                ...
                .addFields(chosen_module.commands.map((n) => {
                return {
                    name: n.name,
                    value: `\`${n.syntax}\`\n${n.explanation}`,
                    inline: false,
        ...
        try {
            ...
            console.log(e);
            msg.channel.send(util_functions.desc_embed('```\n' + e.toString().substring(0, 1000) + '```'));
        }
                    ...
                    if (registered_command.matcher(results[0][0])) {
                        try {
                            if (registered_command.permissions(msg))
                                await registered_command.responder(msg, results[0][0], client, db);
                        }
                        catch (e) {
                            if (e.type == 'user')
                                await msg.channel.send(util_functions.desc_embed(e.message));
                            else if (e.type == 'bot') {
                                await msg.channel.send('An error has occurred. Would you please explain what you were trying to do?');
                                let feedback = await msg.channel.awaitMessages((n) => n.author.id == msg.author.id, { max: 1, time: 20000 });
                                Sentry.configureScope(function (scope) {
                                ...
                                await msg.channel.send(util_functions.desc_embed("There's been an error! Luckily, it's not your fault. Please give the bot owner this ID: " +
                                    Sentry.captureException(e)));
                            }
                            else {
                                console.log(e);
                                if (e.httpStatus === 403) {
                                    await msg.channel.send(util_functions.desc_embed(`**Sorry! ModBot doesn't have permission to do that! Maybe check on my permission settings? Currently ModBot works best with the Administrator permission and must be as close to the top of the role list as possible**\nError: ${e}`));
                                }
                                else {
                                    await msg.channel.send('An error has occurred. Would you please explain what you were trying to do?');
                                    let feedback = await msg.channel.awaitMessages((n) => n.author.id == msg.author.id, { max: 1, time: 30000 });
                                    Sentry.configureScope(function (scope) {
                                        scope.setTag('command', results[0][0].command);
                                        scope.setUser({
                                            id: msg.author.id.toString(),
                                    ...
                                    await msg.channel.send(util_functions.desc_embed('This error is likely not your fault. Please give the bot owner this ID: ' +
                                        Sentry.captureException(e)));
                                }
                            }
                        }
...
if (process.env.STATUSTRACKER_URL) {
    let reportStatus = async () => {
        try {
            let res = await (await nodefetch(process.env.STATUSTRACKER_URL + '/ping', {
                method: 'POST',
                headers: {
